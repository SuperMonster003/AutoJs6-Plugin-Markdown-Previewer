...
class MarkdownPreviewerDocumentNavigationInstrumentationTest {
    ...
    fun linkedDocumentOpensInsideViewerAndBackReturnsToInitialDocument() {
        ...
        val fixtureDirectory = File(
            ...
            "markdown-previewer-instrumentation/navigation",
        ).apply { mkdirs() }
        ...
        val preferences = MarkdownPreviewerPreferences(targetContext)
        val originalFullscreenPreference = preferences.startInFullscreenMode
        preferences.startInFullscreenMode = false

        var activity: MarkdownPreviewerActivity? = null
        try {
            activity = instrumentation.startActivitySync(
                validPreviewerIntent(
                    targetContext.packageName,
                    initialUri,
                    parentUri,
                    initialFile.length(),
                ),
            ) as MarkdownPreviewerActivity
            val webView = activity.findViewById<WebView>(R.id.previewer_web_view)
            val initialUrl = requireNotNull(
                MarkdownPreviewerDocumentLinkPolicy.buildVirtualUrl(INITIAL_FILE_NAME),
            )
            assertTrue(
                "Initial Markdown Previewer did not finish loading",
            ...
            val linkedUrl = requireNotNull(
                MarkdownPreviewerDocumentLinkPolicy.buildVirtualUrl(LINKED_FILE_NAME, LINKED_FRAGMENT),
            )
            assertTrue(
                "Linked Markdown Previewer did not finish loading",
                waitUntil(instrumentation) {
    ...
    private fun validPreviewerIntent(
        packageName: String,
        documentUri: Uri,
        parentUri: Uri,
        size: Long,
        ...
        return Intent(ExplorerActionPluginActions.EXECUTE)
            .setClassName(packageName, MarkdownPreviewerActivity::class.java.name)
