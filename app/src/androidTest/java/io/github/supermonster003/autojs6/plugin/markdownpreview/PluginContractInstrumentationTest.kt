...
class PluginContractInstrumentationTest {
    ...
    fun pluginInfoDeclaresAbiIndependentExplorerEngine() {
        ...
        assertEquals(MarkdownPreviewPlugin.ID, info.id)
        ...
        assertEquals(
            MarkdownPreviewPlugin.REQUIRED_HOST_VERSION,
    ...
    fun catalogUsesParcelableBundleAndStringArrayLists() {
        val catalog = markdownPreviewActionCatalog()
        ...
        assertEquals(MarkdownPreviewPlugin.ID, action?.getString(ExplorerActionCatalogKeys.ID))
        assertEquals(
            MarkdownPreviewPlugin.ACTIVITY_CLASS_NAME,
    ...
    fun manifestProtectsAndExportsTheDiscoveryAndExecutionComponents() {
        ...
        val activityInfo = packageManager.getActivityInfo(
            ComponentName(context, MarkdownPreviewActivity::class.java),
            0,
        )
        ...
        assertTrue(execution.any { it.activityInfo.name == MarkdownPreviewActivity::class.java.name })
    }
}
