...
internal class MarkdownPreviewerTextReader(
    ...
    fun read(uri: Uri, maxBytes: Int): String {
        ...
        val bytes = contentResolver.openInputStream(uri)?.use { input ->
            MarkdownPreviewerTextCodec.readBounded(input, maxBytes)
        } ?: throw IOException("Cannot open the previewer document")
        return MarkdownPreviewerTextCodec.decode(bytes)
    }
}

internal object MarkdownPreviewerTextCodec {
...
internal class MarkdownTooLargeException(
    val limitBytes: Int,
) : IOException("Markdown exceeds the $limitBytes-byte previewer limit")
