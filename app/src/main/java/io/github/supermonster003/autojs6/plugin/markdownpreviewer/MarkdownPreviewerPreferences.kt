...
import io.github.supermonster003.autojs6.plugin.markdownpreviewer.R
import java.io.ByteArrayOutputStream
import java.io.IOException
import java.io.InputStream

enum class MarkdownPreviewerTheme(
    val id: String,
    @param:StringRes val labelRes: Int,
) {
    GITHUB_AUTO("github_auto", R.string.text_previewer_theme_github_auto),
    GITHUB_LIGHT("github_light", R.string.text_previewer_theme_github_light),
    GITHUB_DARK("github_dark", R.string.text_previewer_theme_github_dark),
    PAPER("paper", R.string.text_previewer_theme_paper),
    SEPIA("sepia", R.string.text_previewer_theme_sepia),
    CUSTOM("custom", R.string.text_previewer_theme_custom),
    ;
    ...
    companion object {
        ...
        fun fromId(id: String?): MarkdownPreviewerTheme =
...
class MarkdownPreviewerPreferences(context: Context) {

    private val appContext = context.applicationContext
    private val preferences = appContext.getSharedPreferences(PREFERENCES_NAME, Context.MODE_PRIVATE)
    private val customCssFile = AtomicFile(appContext.filesDir.resolve(CUSTOM_CSS_RELATIVE_PATH))

    var theme: MarkdownPreviewerTheme
        get() {
            val stored = MarkdownPreviewerTheme.fromId(preferences.getString(KEY_THEME, null))
            return stored.takeUnless { it == MarkdownPreviewerTheme.CUSTOM && !hasCustomCss }
                ?: MarkdownPreviewerTheme.GITHUB_AUTO
        }
        set(value) {
            val normalized = value.takeUnless { it == MarkdownPreviewerTheme.CUSTOM && !hasCustomCss }
                ?: MarkdownPreviewerTheme.GITHUB_AUTO
    ...
    var textZoomPercent: Int
        get() = MarkdownPreviewerTextZoom.normalize(
            preferences.getInt(KEY_TEXT_ZOOM_PERCENT, MarkdownPreviewerTextZoom.DEFAULT_PERCENT),
        )
        set(value) = preferences.edit {
            putInt(KEY_TEXT_ZOOM_PERCENT, MarkdownPreviewerTextZoom.normalize(value))
        }
    ...
    fun clearCustomCss() {
        val wasCustom = theme == MarkdownPreviewerTheme.CUSTOM
        customCssFile.delete()
        if (wasCustom) {
            theme = MarkdownPreviewerTheme.GITHUB_AUTO
        }
    }
    ...
    companion object {
        ...
        private const val CUSTOM_CSS_RELATIVE_PATH = "markdown-previewer/custom.css"
