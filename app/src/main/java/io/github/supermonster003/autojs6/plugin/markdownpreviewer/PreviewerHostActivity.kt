...
internal data class PreviewerHostAppearance(val languageTag: String, val darkMode: Boolean) {
    ...
    companion object {
        ...
        fun read(context: Context): PreviewerHostAppearance? = runCatching {
        ...
        fun fromBundle(result: Bundle): PreviewerHostAppearance? {
            ...
            return PreviewerHostAppearance(language, result.getBoolean("darkModeActive"))
        }
    }
}

/** Recreates the plugin's own resource context using the host's resolved language and night mode. */
abstract class PreviewerHostActivity : AppCompatActivity() {
    private var hostAppearance: PreviewerHostAppearance? = null

    override fun attachBaseContext(newBase: Context) {
        hostAppearance = PreviewerHostAppearance.read(newBase)
    ...
    override fun onResume() {
        super.onResume()
        val latest = PreviewerHostAppearance.read(this)
