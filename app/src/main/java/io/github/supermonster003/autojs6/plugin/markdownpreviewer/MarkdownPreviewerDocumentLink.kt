...
internal data class MarkdownPreviewerDocumentLink(
 ...
 */
internal object MarkdownPreviewerDocumentLinkPolicy {

    private const val MAX_ENCODED_PATH_LENGTH = 8 * 1024
    private const val MAX_FRAGMENT_LENGTH = 512

    fun parseVirtualUrl(value: String): MarkdownPreviewerDocumentLink? {
        ...
        if (!uri.rawAuthority.equals(MarkdownPreviewerWebOrigin.DOMAIN, ignoreCase = true)) return null
        if (uri.rawUserInfo != null || uri.port != -1 || uri.rawQuery != null) return null

        val rawPath = uri.rawPath ?: return null
        if (!rawPath.startsWith(MarkdownPreviewerWebOrigin.DOCUMENT_PATH_PREFIX)) return null
        val rawRelativePath = rawPath.removePrefix(MarkdownPreviewerWebOrigin.DOCUMENT_PATH_PREFIX)
        ...
        return MarkdownPreviewerDocumentLink(
    ...
    fun buildVirtualUrl(relativePath: String, fragment: String? = null): String? {
        if (!MarkdownPreviewerPathPolicy.isSafeRelativePath(relativePath)) return null
        if (!isSafeFragment(fragment)) return null
        return runCatching {
            URI(
                "https",
                MarkdownPreviewerWebOrigin.DOMAIN,
                "${MarkdownPreviewerWebOrigin.DOCUMENT_PATH_PREFIX}$relativePath",
                null,
                fragment,
            ).toASCIIString()
        }.getOrNull()
    ...
    private fun decodeRelativePath(rawPath: String): String? {
        ...
        return decodedPath.takeIf(MarkdownPreviewerPathPolicy::isSafeRelativePath)
    }
